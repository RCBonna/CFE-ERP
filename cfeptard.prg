...
#include 'RCB.CH'
 ...
 function CFEPTARD()	// - TERMO DE ABERTURA E ENCERRAMENTO							*
*-----------------------------------------------------------------------------*
local X
...
private PAG       :=PARAMETRO->PA_PAGRD
private LIV       :=PARAMETRO->PA_LIVRD
private VM_EMPR1  :=trim(ASSINATURA[4])
DATA              :={boy(date()),eoy(date())}

nX:=15
pb_box(nX++,25,,,,'Livro Reg.Duplicata')
...
if if(lastkey()#27,pb_ligaimp(I15CPP),.F.)
	...
	*------------------------------------------------------------------*
	...
	VM_TEXTO+=replicate(chr(13)+chr(10),5)
	VM_TEXTO+=padc('T  E  R  M  O     D  E     A  B  E  R  T  U  R  A',115)
	VM_TEXTO+=replicate(chr(13)+chr(10),4)
	VM_TEXTO+=padc('L i v r o   R e g i s t r o   d e   D u p l i c a t a s     N . '+VM_NRDIA,115)
	VM_TEXTO+=replicate(chr(13)+chr(10),3)
	...
	VM_TEXTO+='"REGISTRO DE DUPLICATAS" n. &VM_NRDIA para a empresa &VM_EMPR1 , '
	...
	*------------------------------------------------------------------*
	...
	VM_TEXTO+='"REGISTRO DE DUPLICATAS" n. &VM_NRDIA '
